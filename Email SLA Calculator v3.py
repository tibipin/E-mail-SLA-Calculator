...
class EmailSLACalculator:
    ...
    def extract_mails(self):
        ...
        if len(self.df) != 0:
            self.df['SentOn'] = self.df['SentOn'].str[:19]
    ...
    def extraction_func(self, outlook_folder, filtru_timp, sender_id, coduri_angajati):
        ...
        if messages.count > 0:
            for m in messages:
                try:
                    variabila_eroare = m.SenderEmailAddress
                except:
                    continue
                try:
    ...
    def extraction_algo(self, tower, root_folder):
        ...
        if tower == 'MD':
            ...
            if self.inbox_name == 'procurementGBS':
                target_folders = ['Inbox->Completed All', 'Sent Items']
            elif self.inbox_name == 'GM BPO-Molson.APProcessing':
                target_folders = ['Inbox', 'Sent Items', 'Arina->Waterloo Brewing',
                                  'Arina->Stryder', 'Arina->Novexco', 'Arina->Ingredion', 'Arina->Heineken',
                                  'Arina->Crown Metal', 'Arina->Atlantic Packaging', 'Dana->VideoJet',
                                  'Dana->Samuel Packaging', 'Dana->O-I', 'Dana->Oceanex', 'Dana->ISS Facility',
                                  'Dana->Fastenal', 'Dana->Desjardins', 'Dana->Air Liquide', 'Madalina->Westrock',
                                  'Madalina->Cargill']
            elif self.inbox_name == 'BPO-CBL.Accounts Payable Processing':
                target_folders = ['Inbox', 'Inbox->Actioned Emails', 'Sent Items']
            else:
